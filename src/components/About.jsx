        ...
        <div className="w-full lg:w-1/2">
          ...
          <ul className="grid grid-cols-2 gap-2 text-gray-700 mb-6 list-inside justify-items-start">
            {services.map((service, index) => (
              <li key={index} className="flex items-start">
                <i className="fa-solid fa-broom text-sky-800 mr-2 mt-1"></i>
                {service}
              </li>
            ))}
          </ul>
          <div className="flex justify-center md:justify-start">
            <motion.a
              ...
              className="inline-block bg-sky-500 text-white px-6 py-3 rounded-lg shadow hover:bg-blue-700"
              initial={{ opacity: 0, x: -25 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }} // animate only once
              transition={{ duration: 0.9, ease: [0.17, 0.55, 0.55, 1] }}
            >
              <i className="fab fa-whatsapp mr-2"></i> Book your Slots
            </motion.a>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-wrap mx-4">
            <div className="w-1/2 sm:w-1/2 md:w-1/2 lg:w-1/3 px-4 mb-6">
              <div className="counter-wrap text-center">
                <div className="counter text-5xl font-bold text-sky-800 mb-2">
                  <CountUp start={0} end={10} duration={3} suffix="+" enableScrollSpy scrollSpyOnce>
                    {({ countUpRef }) => (
                      <span ref={countUpRef} className="text-5xl text-sky-800" />
                    )}
                  </CountUp>
                </div>
